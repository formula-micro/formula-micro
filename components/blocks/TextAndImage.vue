...
<template>
    <div v-if="__typename === 'text_and_image_blocks'" class="w-full px-6 xl:px-12">
        <div :class="[ has_vertical_padding ? 'py-4' : '', 'flex justify-center items-center w-full' ]">
            <div :class="[ width?.class, image_placement === 'right' ? 'xl:grid-cols-[3fr,0.5fr,3fr]' : 'xl:grid-cols-[3fr,0.5fr,3fr]', has_vertical_padding ? 'py-8 xl:py-16' : '', has_horizontal_padding ? 'px-6' : '', 'grid w-full grid-cols-1 gap-x-8 gap-y-6 lg:gap-y-0 items-center' ]">
