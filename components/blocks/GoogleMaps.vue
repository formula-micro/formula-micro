<script setup lang="ts">
    ...
    const isGoogleMapsVisible = computed (() => cookiesAccepted.value ? cookiesAccepted.value.split(',').includes('googleMap') : false);
</script>
...
<template>
    <div :class="[ has_vertical_padding ? 'py-8' : '', has_horizontal_padding ? 'xl:px-12' : '','flex justify-center items-center w-full' ]">
        <div :class="[ width?.class, has_horizontal_padding ? 'px-6 xl:px-0' : '', 'w-full' ]">
            <!-- Google Maps -->
            <div v-if="isGoogleMapsVisible" class="w-full h-96">
