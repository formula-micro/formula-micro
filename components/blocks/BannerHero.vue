    ...
    <div v-if="__typename === 'banner_hero_blocks'" class="w-full px-6 xl:px-12">
        <div class="flex justify-center items-center w-full">
            <div :class="[ width?.class ]">
                <Splide :options="{ rewind: true, drag: banners ? banners.length > 1 : false, autoplay: autoplay, interval: interval, video: { playerOptions: { htmlVideo: { preload: 'auto' } } } }" :extensions="{ Video }" :has-track="false">
...
<template>
    <div v-if="__typename === 'banner_hero_blocks'" class="w-full px-6 xl:px-12">
        <div class="flex justify-center items-center w-full">
            <div :class="[ width?.class ]">
                <Splide :options="{ rewind: true, drag: banners?.length > 1, video: { playerOptions: { htmlVideo: { preload: 'auto' } } } }" :extensions="{ Video }" :has-track="false">
                    <SplideTrack>
                        <SplideSlide v-for="banner in banners" :key="banner?.id" :data-splide-html-video="'https://cms.formula.nu/assets/' + banner?.image?.id">
                    ...
                    <div :class="[ banners?.length > 1 ? 'hidden xl:block' : 'hidden', 'splide__arrows' ]">
                        <button type="button" class="splide__arrow--prev absolute flex justify-center items-center rounded-full bg-white opacity-70 top-1/2 transform -translate-y-1/2 rotate-180 p-1.5">
                            <Icon name="tabler:arrow-left" class="h-5 w-5 text-gray-900" />
                        </button>
